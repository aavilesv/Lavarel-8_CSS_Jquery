			...
			<div class="card full-height">
				<div class="card-body">
					...
					<div class="d-flex flex-wrap justify-content-around pb-2 pt-4">
						<form action="{{route('provincias.update',$provincia)}}" method="POST" class="form-control form-createe">
							<!--estogenera el token de validacion  -->
							@csrf
							@method('put')
							<div class="form-group">
								<label for="name">Nombre:</label>
								<input type="text" name="name"  class="form-control"  onKeyPress="return sololetrascoma(event)" 
								 placeholder="Nombre"
								value="{{old('name',$provincia->name)}}">
								@error('name')
