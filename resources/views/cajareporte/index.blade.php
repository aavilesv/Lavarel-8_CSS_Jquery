...
@section('title', 'Listado de Reportes de Caja')
@section('content')
                                ...
                                <div class="form-button-action">
                                    <div class="row">
                                        ...
                                        @if ($buscador != '' or $buscarincial != '')
                                            <div class="col-md-2">
                                                <a href="{{ route('reportecaja.index') }}" data-toggle="tooltip"
                            ...
                            <div class="table-responsive">
                                <table class="display table table-striped table-hover nowrap">
                                    ...
                                    <tbody>
                                        @forelse ($caja as $caj)
                                            <tr>
                                                <td class="success">{{ $caj->id }}</td>
                                                <td>{{ $caj->fecha }}</td>
                                                <td>${{ $caj->cajaprincipal }}</td>
                                                <td>${{ $caj->cajafinal }}</td>
                                                <td>${{ $caj->saldocaja }}</td>
                                                <td>${{ $caj->saldoingeniero }}</td>
                                                <td>{{ $caj->created_at }}</td>
                                                <td>
                                                    <div class="form-button-action">
                                                        <a class="btn btn-link btn-success btn-lg" data-toggle="tooltip"
                                                            data-original-title="Ver"
                                                            href="{{ route('reportecaja.show', $caj->id) }}"><i
                                                                class="fa fa-eye"></i></a>
                                ...
                                {{ $caja->appends(['buscador' => $buscador, 'buscarincial' => $buscarincial, 'buscarfinal' => $buscarfinal])->links() }}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="{{ route('reportedit.store') }}" class="form-createe" >
                        <div class="row">
                            @csrf
                            <div class="col-sm-12">
